...
if(isset($_POST['order_btn'])){
    ...
    $email = $_POST['email'];
    $method = $_POST['method'];
    $address = $_POST['line-one'].', '. $_POST['street'].', '. $_POST['city'].', '. $_POST['country'].' - '. $_POST['pin_code'];
    $placed_on = date('d-M-Y');

    $cart_total = 0;
    $cart_products[] = '';

    $sql3 = "SELECT * FROM cart WHERE UID = '$user_UID'";
    $cart_query = mysqli_query($conn, $sql3) or die('query failed');
    ...
    $sql4 = "SELECT * FROM orders WHERE Name = '$name' AND Number = '$number' AND Email = '$email' AND Method = '$method' 
                AND Address = '$address' AND TotalItems = '$total_products' AND TotalPrice = '$cart_total'";
    $order_query = mysqli_query($conn, $sql4) or die('query failed');
    ...
    if($cart_total == 0){
        ...
        if(mysqli_num_rows($order_query) > 0){
            ...
            $sql5 = "INSERT INTO orders(UID, Name, Number, Email, Method, Address, TotalItems, TotalPrice, PlacedOn) 
                        VALUES('$user_UID', '$name', '$number', '$email', '$method', '$address', '$total_products', '$cart_total', '$placed_on')";
            mysqli_query($conn, $sql5) or die('query failed');
            $message[] = 'Order Placed Successfully!';
            $sql6 = "DELETE FROM cart WHERE UID = '$user_UID'";
...
<html lang="en">
<head>
    <title>OnlyBooks - Checkout</title>
    <meta charset="UTF-8">
...
<section class="display-order">
    ...
    if(mysqli_num_rows($select_cart) > 0){
        while($fetch_cart = mysqli_fetch_assoc($select_cart)){
            ...
            ?>
            <p> <?php echo $fetch_cart['ItemName']; ?> <span>(<?php echo '$'.$fetch_cart['Price'].'.00'.' x '. $fetch_cart['Quantity']; ?>)</span> </p>
            <?php
        }
    }else{
        echo '<p class="empty">Your Cart is Empty! <a href="shop.php" class="empty-link" style="text-decoration: none;">Shop Now</a></p>';
    }
    ?>
...
<section class="checkout">
    ...
    <?php
    $sql7 = "SELECT * FROM systemusers WHERE UID = '$user_UID'";
    $select_user_info = mysqli_query($conn, $sql7) or die('query failed');
    if (mysqli_num_rows($select_user_info) > 0){
        while($fetch_user = mysqli_fetch_assoc($select_user_info)){
            ?>
            <form action="" method="post">
                ...
                <div class="flex">
                    <div class="inputBox">
                        <span>Your Name :</span>
                        <input type="text" name="name" required value="<?php echo $fullName?>">
                    </div>
                    <div class="inputBox">
                        <span>Phone Number :</span>
                        <input type="text" name="number" required value="<?php echo $fetch_user['PhoneNo']?>">
                    </div>
                    <div class="inputBox">
                        <span>Your Email :</span>
                        <input type="email" name="email" required value="<?php echo $fetch_user['Email']?>">
                    </div>
                    <div class="inputBox">
                    ...
                    <div class="inputBox">
                        ...
                        <input type="text" name="line-one" required value="<?php echo $fetch_user['Address']?>" ">
                    </div>
                    <div class="inputBox">
                        <span>Address line 02 :</span>
                        <input type="text" name="street" required placeholder="E.g M2 Ring Rd">
                    </div>
                    <div class="inputBox">
                        <span>City :</span>
                        <input type="text" name="city" required value="<?php echo $fetch_user['City']?>">
                    </div>

                    <div class="inputBox">
                        <span>Country :</span>
                        <input type="text" name="country" required placeholder="E.g Trinidad">
                    </div>
                    <div class="inputBox">
                ...
                <input type="submit" value="order now" class="btn btn-accent" name="order_btn">
            </form>
